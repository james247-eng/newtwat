...
import { initApiManager, loadApiKeys, closeModals } from './api-manager.js';
...
async function handleGenerate() {
  ...
  try {
    ...
    if (result.data.success) {
      showStatus(`Speech generated! (${result.data.charactersUsed} characters used)`, 'success');
      
      // Play audio
      playAudio(result.data.audioUrl, result.data.audioId);
      
      // Reload history
      await loadAudioHistory();
      
      // Reload API keys to update usage
      await loadApiKeys();
...
async function showUsageStats() {
  ...
  try {
    ...
    if (result.data.success) {
      renderUsageStats(result.data.stats);
    } else {
